...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        if (state, action) not in self.Q_values:
            return 0.0
        return self.Q_values.__getitem__((state, action))
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actions:
           if self.getQValue(state, action)>maxi:
              maxi=self.getQValue(state, action)
        return maxi
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for i in self.featExtractor.getFeatures(state, action):
            Q_val+=self.weights[i] * self.featExtractor.getFeatures(state, action)[i]
        return Q_val
    ...
    def update(self, state, action, nextState, reward):
        ...
        for i in self.featExtractor.getFeatures(state, action):
            self.weights[i]=self.weights[i]+self.alpha*difference*self.featExtractor.getFeatures(state,action)[i]
